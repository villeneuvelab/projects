...
**Date modified**: November 9th 2022
...
- Bash was used to launch both Python and R processes from the command-line, when possible
...
**Warning**: the feature selection step is quite computationally intensive and not optimized. As a result, running this step can take quite some time, which increase with the number of participants and the number of edges.
...
- `edgewise_intraclass_correlation/python/post_icc_calculation.py`: Main script computing the measures needed for the plots of the ICC measures. Script is a set of functions, but doesn't support command-line arguments. Each public function needs to be called in order for the script to work. Functions have Cam-CAN specific quirks that need modification before use in other studies (e.g., number of windows set).

### R scripts
- `edgewise_intraclass_correlation/r/icc_measures_plots.Rmd`: R script to plot all edgewise related results. Not command-line launchable and several Cam-CAN specific quirks.

## **Functional connectivity variance**
*NOTE*: Most of the scripts used for the functional connectivity variance were re-used for the temporal similarity profile analysis, with the exception of the bash files to launch the R scripts.

### Python scripts
- `fc_variance/python/variance_calculator.py`: Script computing within-individual and between-individual variability in FC between individuals. Fully command-line executable.

### R scripts
- `fc_variance/r/var_age.R`: Script computing the relationship between variance in FC and age. Fully command-line executable.
- `fc_variance/r/var_ica.R`: Script computing the relationship between variance in FC and brain volume. Fully command-line executable.

### Bash scripts
- `fc_variance/bash/var_age.sh`: Helper bash file launching the R script `var_age.R` from the command-line with different arguments.
- `fc_variance/bash/var_ica.sh`: Helper bash file launching the R script `var_ica.R`

## **Temporal profile similarity**

### Python
**NOTE:** Variability in temporal similarity profile is calculated with `fc_variance/python/variance_calculator.py` since the shape of the temporal similarity matrices are the same as the FC matrices and since we want to use a similar measure.
- `temp_similarity/python/hctsa_process.py`: Python script taking the normalized output from the hctsa toolbox and generating the temporal profile similarity for each participant (shape similar to FC matrix).  

### R script
**Note:** R scripts used are the ones from the fc_variance section.

### Bash script
- `temp_similarity/bash/hctsa_age.sh`: Bash helper script to launch the `var_age.R` script with the temporal similarity profile-specific arguments.
- `temp_similarity/bash/hctsa_ica.sh`: Bash helper script to launch the `var_ica.R` script with the temporal similarity profile-specific arguments.

### Matlab script
Will be added shortly to the repository.

## Other
Collection of other scripts used in the coding process that may be useful.
- `others/parallel_echo_command.sh`: To avoid generating manually all the arguments necessary to launch the fingerprinting scripts on the Beluga supercomputer, I made a short script that echos all the possible permutations of arguments. The generated file is used to splits each call to the script in a separate job on Beluga.
- `others/ggseg_brain_plot.Rmd`: Simple R script to generate images of the Yeo network (for visualization purposes only).